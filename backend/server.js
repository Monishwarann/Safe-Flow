...
// ============== FUTURE AI & ECO INNOVATION ROUTES ==============
app.get('/api/routes/predictive-traffic', authMiddleware, (req, res) => {
    const horizonHours = [1, 3, 6];
    const trafficCurrent = generateTrafficData();
    const predictions = trafficCurrent.map(zone => {
        const h1 = Math.min(100, Math.max(10, zone.congestionLevel + Math.floor(Math.random() * 16 - 8)));
        const h3 = Math.min(100, Math.max(10, zone.congestionLevel + Math.floor(Math.random() * 26 - 13)));
        const h6 = Math.min(100, Math.max(10, zone.congestionLevel + Math.floor(Math.random() * 34 - 17)));

        return {
            zoneId: zone.zoneId,
            zoneName: zone.zoneName,
            currentCongestion: zone.congestionLevel,
            forecast1h: h1,
            forecast3h: h3,
            forecast6h: h6,
            trend: h3 > zone.congestionLevel ? 'Worsening' : 'Improving',
            confidenceScore: Math.round((88 + Math.random() * 9) * 10) / 10, // AI confidence %
            suggestedAction: h3 > 70 ? 'Activate Signal Optimization' : 'Normal Flow Monitoring',
        };
    });
    res.json({ predictions, generatedAt: new Date().toISOString() });
});

app.post('/api/routes/emergency-corridor', authMiddleware, (req, res) => {
    const { vehicleType = 'Ambulance', originZone, destinationZone } = req.body;
    const origin = CITY_ZONES.find(z => z.id === originZone || z.name === originZone) || CITY_ZONES[0];
    const dest = CITY_ZONES.find(z => z.id === destinationZone || z.name === destinationZone) || CITY_ZONES[2];

    const corridorId = `CORRIDOR-${Math.floor(1000 + Math.random() * 9000)}`;
    const signalsOverridden = Math.floor(4 + Math.random() * 5);
    const estimatedTimeSavedMin = Math.floor(8 + Math.random() * 7);
    ...
    res.json({
        ...
        signalsSynchronized: signalsOverridden,
        estimatedTimeSavedMin,
        priorityLevel: 'HIGH_PRIORITY_PREEMPTION',
        activeWaypoints: generateWaypoints({ lat: origin.lat, lng: origin.lng }, { lat: dest.lat, lng: dest.lng }, 6),
        dispatchTimestamp: new Date().toISOString(),
    });
});

app.post('/api/emissions/offset-calculator', authMiddleware, (req, res) => {
    const { monthlyKm = 500, vehicleType = 'car' } = req.body;
    const factors = { car: 0.21, suv: 0.28, truck: 0.45, electric: 0.05, hybrid: 0.11 };
    const factor = factors[vehicleType] || factors.car;

    const monthlyCO2Kg = Math.round(monthlyKm * factor * 100) / 100;
    const yearlyCO2Tonnes = Math.round((monthlyCO2Kg * 12 / 1000) * 100) / 100;
    const treesNeededToOffset = Math.ceil(monthlyCO2Kg * 12 / 21.77);
    const ecoCreditsEarnable = Math.round(monthlyKm * 0.4);
    ...
    res.json({
        ...
        ecoCreditsEarnable,
        suggestedOffsets: [
            { provider: 'City Green Canopy Program', costUsd: Math.round(treesNeededToOffset * 2.5), treesPlanted: treesNeededToOffset },
            { provider: 'Urban Solar Grid Fund', costUsd: Math.round(yearlyCO2Tonnes * 18), co2NeutralizedTonnes: yearlyCO2Tonnes },
        ],
    });
});
...
app.get('/api/gamification/leaderboard', authMiddleware, (req, res) => {
    const leaderboard = [
        { rank: 1, name: 'EcoTransit Fleet A', category: 'Fleet', co2SavedKg: 1420, ecoPoints: 4850, badge: '🏆 Green Giant' },
        { rank: 2, name: 'Metro Commuters Club', category: 'Community', co2SavedKg: 1180, ecoPoints: 3920, badge: '🥇 Zero Emission Champion' },
        { rank: 3, name: 'Downtown EV Express', category: 'Public Transit', co2SavedKg: 950, ecoPoints: 3100, badge: '🥈 EV Pioneer' },
        { rank: 4, name: 'John Citizen', category: 'Individual', co2SavedKg: 430, ecoPoints: 1650, badge: '🥉 Eco Warrior' },
        { rank: 5, name: 'Green Campus Commute', category: 'Education', co2SavedKg: 380, ecoPoints: 1290, badge: '🌿 Sustainable Voyager' },
    ];
    res.json({ leaderboard });
});

app.post('/api/routes/multimodal', authMiddleware, (req, res) => {
    const { origin, destination } = req.body;
    const dist = origin && destination ? Math.sqrt(Math.pow(destination.lat - origin.lat, 2) + Math.pow(destination.lng - origin.lng, 2)) * 111 : 12;

    res.json({
        options: [
            {
                type: 'EV Bus + Metro',
                totalTimeMin: Math.round(dist * 2.8),
                co2SavedKg: Math.round(dist * 0.18 * 100) / 100,
                costUsd: 2.50,
                steps: ['5 min E-Scooter to Metro', '12 min Metro Line 1', '8 min Electric Bus Route 4']
            },
            {
                type: 'Micro-Mobility (E-Bike Only)',
                totalTimeMin: Math.round(dist * 3.5),
                co2SavedKg: Math.round(dist * 0.21 * 100) / 100,
                costUsd: 1.80,
